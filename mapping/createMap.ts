...
function getLocationsStyle(vectorSource: VectorSource<Feature<Geometry>>) {
...
function getNewArticlesStyle(vectorSource: VectorSource<Feature<Geometry>>) {
  return new VectorLayer({
    source: vectorSource,
    style: new Style({
      image: new Icon({
...
function addMapClickEventListener(
  ...
  const hoverHandler = (event: MapBrowserEvent) => {
    const pixel = event.pixel;
    const feature = map.forEachFeatureAtPixel(pixel, (feature) => {
      return feature;
    });
...
function spawnLocationsOnMap(
  ...
  const locationsVectorSource = new VectorSource({
